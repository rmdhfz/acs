...
**[BELUM DIPERBAIKI, TINGGI — ditemukan 2026-08-22 saat review dashboard, bukan regresi] `GET /tasks` dan `GET /tasks/:id` sama sekali tidak tenant-scoped.**
`domain.TaskFilter` tidak punya `TenantID`; `listTasks`/`getTask` (`internal/delivery/http/task_handler.go`) tidak pernah memanggil `ActorFrom(c)`; `task.Service.List()`/`Get()` tidak menerima `actor` sama sekali — beda dari `device.Service.List()`/`Get()` yang sudah pakai `requireTenantScope()`. Dampak: user non-superadmin dari tenant mana pun bisa membaca task milik tenant lain (`GET /tasks?device_id=<milik tenant lain>` atau tebak `GET /tasks/123` — ID sequential, gampang di-enumerasi), termasuk `tasks.parameters` yang bisa memuat nilai konfigurasi CPE sensitif (mis. WiFi passphrase/PPPoE credential yang sedang di-push via `SetParameterValues`). Ikut terlihat lebih jelas sekarang karena endpoint baru `GET /tasks/stats` (dashboard) justru SUDAH benar tenant-scoped — jadi API `/tasks/*` sekarang punya isolasi yang tidak konsisten antar endpoint. Perbaikan: tambah `TenantID` ke `TaskFilter`, ubah `task.Service.List/Get` terima `actor` dan terapkan `requireTenantScope`-style check (device milik task di-resolve dulu utk tahu tenant_id-nya), update handler pasang `ActorFrom(c)`. Belum dikerjakan — menunggu konfirmasi user karena mengubah signature method yang sudah dipakai di beberapa tempat (`usecase/session`, `usecase/firmware`, `usecase/diagnostics` semua punya `TaskCreator`/pemanggil task service).
