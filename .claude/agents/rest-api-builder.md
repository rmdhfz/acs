...
2. **RBAC:** role yang berlaku — `SUPERADMIN`, `ADMIN` (scoped per tenant), `NOC`, `VIEWER`, `ENDUSER` (pelanggan akhir; hanya boleh menyentuh device miliknya sendiri lewat jalur self-service, lihat `internal/usecase/selfservice/` dan pemetaan `user_devices`). Konstanta resminya ada di `internal/domain/common.go` (`RoleSuperadmin`, `RoleAdmin`, `RoleNOC`, `RoleViewer`, `RoleEndUser`) — pakai konstanta itu, jangan menulis literal string role di handler. Cek scope tenant di middleware pada setiap request, bukan cuma cek role. Data device, profil, firmware harus terpisah logis per tenant (FR-26) — jangan biarkan query lolos tanpa filter tenant_id, kecuali endpoint yang memang eksplisit lintas-tenant untuk `SUPERADMIN` (mis. kelola vendor/parameter mapping global — FR-28).
